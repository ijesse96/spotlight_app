...
/// Local queue controller implementation
class LocalQueueController extends BaseQueueController {
  final QueueConfig _config;
  final UnifiedTimerService _timerService = UnifiedTimerService();

  LocalQueueController(this._config);
  ...
  Future<void> joinQueue() async {
    ...
    print('🔄 [LOCAL_QUEUE] Joining local queue: ${_config.displayName}');
    print('✅ [LOCAL_QUEUE] User: $displayName');

    try {
      // Remove user from all other queues first
      await _removeUserFromAllQueues(userId);

      // Add user to local queue
      await queueCollection.doc(userId).set({
        ...
        'locationId': _config.id,
        'locationName': _config.displayName,
        'queueType': 'local',
      });

      print('✅ [LOCAL_QUEUE] Successfully joined local queue: ${_config.displayName}');
    } catch (e) {
      print('❌ [LOCAL_QUEUE] Error joining local queue: $e');
      rethrow;
    }
  }
  ...
  Future<void> leaveQueue() async {
    ...
    final userId = currentUserId!;
    print('🔄 [LOCAL_QUEUE] Leaving local queue: ${_config.displayName}');
    ...
    try {
      ...
      print('✅ [LOCAL_QUEUE] Successfully left local queue: ${_config.displayName}');
    } catch (e) {
      print('❌ [LOCAL_QUEUE] Error leaving local queue: $e');
      rethrow;
    }
  }
  ...
  Future<void> setUserAsLive(String userId, String userName) async {
    print('🔄 [LOCAL_QUEUE] Setting user as live: $userName');
    ...
    try {
      ...
      print('✅ [LOCAL_QUEUE] User set as live: $userName');
    } catch (e) {
      print('❌ [LOCAL_QUEUE] Error making user live: $e');
      rethrow;
    }
  }
  ...
  Future<void> endLiveSession() async {
    print('🔄 [LOCAL_QUEUE] Ending live session');
    ...
    try {
      ...
      print('✅ [LOCAL_QUEUE] Live session ended');
    } catch (e) {
      print('❌ [LOCAL_QUEUE] Error ending live session: $e');
      rethrow;
    }
  }
  ...
  Future<void> moveToNextUser() async {
    print('🔄 [LOCAL_QUEUE] Moving to next user');
    ...
    try {
      ...
      if (queueSnapshot.docs.isEmpty) {
        print('ℹ️ [LOCAL_QUEUE] No users in queue');
        await endLiveSession();
        return;
      }
      ...
      print('✅ [LOCAL_QUEUE] Moved to next user: $nextUserName');
    } catch (e) {
      print('❌ [LOCAL_QUEUE] Error moving to next user: $e');
      rethrow;
    }
  }
  ...
  Future<void> _removeUserFromAllQueues(String userId) async {
    try {
      ...
      print('🔄 [LOCAL_QUEUE] Removed user $userId from all queues');
    } catch (e) {
      // Ignore errors if user wasn't in queue
      print('ℹ️ [LOCAL_QUEUE] User not in queue or already removed');
    }
  }
  ...
  Future<Map<String, dynamic>> getQueueStats() async {
    try {
      ...
      print('❌ [LOCAL_QUEUE] Error getting queue stats: $e');
      return {};
    }
  }
  ...
  /// Get local-specific information
  String get locationId => _config.id;
  String get locationName => _config.displayName;
  String get localQueueType => 'local';
} 