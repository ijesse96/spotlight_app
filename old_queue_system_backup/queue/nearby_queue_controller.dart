...
/// Nearby queue controller implementation
class NearbyQueueController extends BaseQueueController {
  final QueueConfig _config;
  final UnifiedTimerService _timerService = UnifiedTimerService();

  NearbyQueueController(this._config);
  ...
  Future<void> joinQueue() async {
    ...
    print('🔄 [NEARBY_QUEUE] Joining nearby queue: ${_config.id}');
    print('✅ [NEARBY_QUEUE] User: $displayName');

    try {
      // Remove user from all other queues first
      await _removeUserFromAllQueues(userId);

      // Add user to nearby queue
      ...
      print('✅ [NEARBY_QUEUE] Successfully joined nearby queue: ${_config.id}');
    } catch (e) {
      print('❌ [NEARBY_QUEUE] Error joining nearby queue: $e');
      rethrow;
    }
  }
  ...
  Future<void> leaveQueue() async {
    ...
    final userId = currentUserId!;
    print('🔄 [NEARBY_QUEUE] Leaving nearby queue: ${_config.id}');

    try {
      // Remove from nearby queue
      await queueCollection.doc(userId).delete();
      
      // End live session if user was live
      await endLiveSession();
      
      print('✅ [NEARBY_QUEUE] Successfully left nearby queue: ${_config.id}');
    } catch (e) {
      print('❌ [NEARBY_QUEUE] Error leaving nearby queue: $e');
      rethrow;
    }
  }
  ...
  Future<void> setUserAsLive(String userId, String userName) async {
    print('🔄 [NEARBY_QUEUE] Setting user as live: $userName');
    ...
    try {
      ...
      print('✅ [NEARBY_QUEUE] User set as live: $userName');
    } catch (e) {
      print('❌ [NEARBY_QUEUE] Error making user live: $e');
      rethrow;
    }
  }
  ...
  Future<void> endLiveSession() async {
    print('🔄 [NEARBY_QUEUE] Ending live session');
    ...
    try {
      ...
      print('✅ [NEARBY_QUEUE] Live session ended');
    } catch (e) {
      print('❌ [NEARBY_QUEUE] Error ending live session: $e');
      rethrow;
    }
  }
  ...
  Future<void> moveToNextUser() async {
    print('🔄 [NEARBY_QUEUE] Moving to next user');
    ...
    try {
      ...
      if (queueSnapshot.docs.isEmpty) {
        print('ℹ️ [NEARBY_QUEUE] No users in queue');
        await endLiveSession();
        return;
      }
      ...
      print('✅ [NEARBY_QUEUE] Moved to next user: $nextUserName');
    } catch (e) {
      print('❌ [NEARBY_QUEUE] Error moving to next user: $e');
      rethrow;
    }
  }
  ...
  Future<void> _removeUserFromAllQueues(String userId) async {
    try {
      ...
      print('🔄 [NEARBY_QUEUE] Removed user $userId from all queues');
    } catch (e) {
      // Ignore errors if user wasn't in queue
      print('ℹ️ [NEARBY_QUEUE] User not in queue or already removed');
    }
  }
  ...
  Future<Map<String, dynamic>> getQueueStats() async {
    try {
      ...
      print('❌ [NEARBY_QUEUE] Error getting queue stats: $e');
      return {};
    }
  }
  ...
  /// Get nearby-specific information
  String get locationId => _config.id;
  String get nearbyQueueType => 'nearby';
} 