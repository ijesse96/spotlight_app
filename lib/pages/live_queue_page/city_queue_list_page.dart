...
class CityQueueListPage extends StatefulWidget {
  final String cityId;
  final String cityName;
  final String state;

  const CityQueueListPage({
    Key? key,
  ...
  @override
  _CityQueueListPageState createState() => _CityQueueListPageState();
}

class _CityQueueListPageState extends State<CityQueueListPage> with WidgetsBindingObserver {
  Timer? _timer;
  PageController? _photoPageController;
  bool _isInitialized = false;
  bool _isDisposed = false;
  
  // Static map to track initialization per city
  static final Map<String, bool> _cityInitialized = {};
  ...
  // City Queue: City-specific queue for local spotlight
  ...
  void initState() {
    ...
    // Initialize Firestore streams for city queue
    _liveUserStream = _queueService.getCityLiveUser(widget.cityId);
    _queueUsersStream = _queueService.getCityQueueUsers(widget.cityId);
    _timerStream = _queueService.getCityTimer(widget.cityId);
    
    // Initialize timer if it doesn't exist (only once per city)
    if (!_cityInitialized.containsKey(widget.cityId) || !_cityInitialized[widget.cityId]!) {
      _initializeTimer();
      _cityInitialized[widget.cityId] = true;
    }
  ...
  void _attachStreams() {
    ...
    _timerSub = _queueService.getCityTimer(widget.cityId).listen((timerData) {
      if (timerData != null) {
        final isActive = timerData['isActive'] ?? false;
        final countdown = timerData['countdown'] ?? 20;
        
        print('City Timer stream update - Countdown: $countdown, IsActive: $isActive');
        
        // Ensure persistent timer is running if there's a live user and timer is active
        if (isActive) {
          _queueService.ensureCityTimerRunning(widget.cityId);
        }
    ...
    _liveUserSub = _queueService.getCityLiveUser(widget.cityId).listen((liveUser) {
      if (liveUser != null) {
        print('City Live user changed to: ${liveUser['name']}');
        // Ensure persistent timer is running for the new live user
        _queueService.ensureCityTimerRunning(widget.cityId);
      } else {
        print('No city live user');
      }
      setState(() {});
    });
  }
  ...
  void didChangeDependencies() {
    ...
    _attachStreams();
    // Always re-fetch the timer/user streams to ensure up-to-date timer after switching tabs
    _liveUserStream = _queueService.getCityLiveUser(widget.cityId);
    _queueUsersStream = _queueService.getCityQueueUsers(widget.cityId);
    _timerStream = _queueService.getCityTimer(widget.cityId);
  }
  ...
  @override
  void didUpdateWidget(CityQueueListPage oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.cityId != oldWidget.cityId) {
      // City changed, reset and reinitialize timer and streams
      _cityInitialized[widget.cityId] = false;
      _initializeTimer();
      _cityInitialized[widget.cityId] = true;
      _attachStreams();
      // Always re-fetch the timer/user streams to ensure up-to-date timer after switching tabs
  ...
  void _initializeTimer() async {
    print('Initializing timer for city: ${widget.cityId}');
    
    // Check if there's already a live user before initializing
    final liveUser = await _queueService.getCityLiveUser(widget.cityId).first;
    final hasLiveUser = liveUser != null;
    
    if (hasLiveUser) {
      print('Live user exists, checking timer state');
      // Check if timer exists and is active
      final timerData = await _queueService.getCityTimer(widget.cityId).first;
      if (timerData != null) {
        final isActive = timerData['isActive'] ?? false;
        if (isActive) {
          print('Live user exists and timer is active, ensuring persistent timer is running');
          await _queueService.ensureCityTimerRunning(widget.cityId);
        } else {
          print('Live user exists but timer not active, activating timer');
          await _queueService.updateCityTimer(widget.cityId, timerData['countdown'] ?? 20, true);
          // Start the persistent timer
          _queueService.resetCityTimer(widget.cityId);
        }
      } else {
        print('Live user exists but no timer found, creating timer');
        await _queueService.initializeCityTimer(widget.cityId);
        await _queueService.resetCityTimer(widget.cityId);
      }
  ...
  Future<void> _cleanupEmptyNamesInQueue() async {
    try {
      final queueUsers = await _queueService.getCityQueueUsers(widget.cityId).first;
      for (var user in queueUsers) {
        ...
        if (name == null || name.isEmpty || userId == null) {
          ...
          await FirebaseFirestore.instance.collection('city_queue_${widget.cityId}').doc(userId).delete();
  ...
  void _listenToTimerChanges() {
    _timerStream?.listen((timerData) {
      if (timerData != null) {
        ...
        if (isActive) {
          // The persistent timer in QueueService handles this
        } else {
          // The persistent timer in QueueService handles this
        }
      } else {
  ...
  // void _startCountdownTimer() { // This method is removed
  //   if (_isDisposed) return;
    
  //   _countdownTimer?.cancel();
  //   print('Starting countdown timer for city: ${widget.cityId}');
  //   _countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
  //     try {
  //       // Check if page is disposed before proceeding
  //       if (_isDisposed) {
  //         timer.cancel();
  ...
  //         print('Timer update - Countdown: $countdown, IsActive: $isActive');
          
  //         if (isActive) {
  //           final newCountdown = countdown - 1;
  //           print('Decrementing timer to: $newCountdown');
            
  //           if (newCountdown <= 0) {
  //             // Timer finished, move to next user
  //             print('Timer finished, moving to next streamer');
  //             timer.cancel();
  //             _countdownTimer = null;
  //             if (!_isDisposed) {
  ...
  //             }
  //           } else {
  //             // Update the timer in Firestore with decremented value
  //             if (!_isDisposed) {
  //               await _queueService.updateCityTimer(widget.cityId, newCountdown, true);
  //               print('Updated Firestore timer to: $newCountdown');
  //             }
  //           }
  //         } else {
  //           print('Timer is not active, stopping countdown');
  //           timer.cancel();
  //           _countdownTimer = null;
  //         }
  //       }
  //     } catch (e) {
  //       print('Error in countdown timer: $e');
  //       timer.cancel();
  //       _countdownTimer = null;
  //     }
  //   });
  ...
  // Future<void> _moveToNextStreamer() async { // This method is removed
  //   // City Queue rotation logic: Move next user to city spotlight
  //   final queueUsers = await _queueService.getCityQueueUsers(widget.cityId).first;
    
  //   if (queueUsers.isNotEmpty) {
  //     // Set the first user as live
  ...
  //     await _queueService.setCityUserAsLive(widget.cityId, nextUser['userId'], displayName);
  //   } else {
  //     // No users in queue, end current live session
  //     await _queueService.endCityLiveSession(widget.cityId);
  //   }
  ...
  void dispose() {
    ...
    _timerSub?.cancel();
    _liveUserSub?.cancel();
    // Reset static flag so timer can be reinitialized if user switches back
    _cityInitialized.remove(widget.cityId);
    super.dispose();
  }
  
  // Static method to reset initialization for a city (useful for testing)
  static void resetCityInitialization(String cityId) {
    _cityInitialized.remove(cityId);
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          StreamBuilder<Map<String, dynamic>?>(
            stream: _queueService.getCurrentUserCityQueueStatus(widget.cityId),
            builder: (context, snapshot) {
                ...
                return Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () => _queueService.joinCityQueue(widget.cityId),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: orangeColor,
                        foregroundColor: Colors.white,
                      ),
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      showDialog(
                        ...
                        builder: (BuildContext context) {
                          return AlertDialog(
                            title: Text("Leave ${widget.cityName} Queue?"),
                            content: Text("If you leave the ${widget.cityName} queue, you'll lose your spot."),
                            actions: [
                              TextButton(
                              ...
                              ElevatedButton(
                                onPressed: () async {
                                  ...
                                  await _queueService.leaveCityQueue(widget.cityId);
                                },
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: orangeColor,
                                  foregroundColor: Colors.white,
                    ...
                    child: Text("Leave ${widget.cityName} Queue"),
                  ),
                ),
              );
            },
