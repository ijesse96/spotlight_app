...
class VLRStreamPage extends StatefulWidget {
  final String roomId;
  ...
  @override
  State<VLRStreamPage> createState() => _VLRStreamPageState();
}

class _VLRStreamPageState extends State<VLRStreamPage> {
  ...
  void initState() {
    ...
    _initializeVLRStream();
  }

  void _initializeVLRStream() async {
    // Initialize VLR-specific stream (only if timer doesn't exist)
    final timerData = await _queueService.getVLRTimer(widget.roomId).first;
    if (timerData == null) {
      await _queueService.initializeVLRTimer(widget.roomId);
    }
    _listenToVLRLiveUser();
    _listenToVLRTimer();
  }

  void _listenToVLRLiveUser() {
    _liveUserSubscription = _queueService.getVLRLiveUser(widget.roomId).listen((liveUser) {
      if (liveUser != null) {
        setState(() {
  ...
  void _listenToVLRTimer() {
    _timerSubscription = _queueService.getVLRTimer(widget.roomId).listen((timerData) {
      if (timerData != null) {
        final countdown = timerData['countdown'] ?? 20;
        final isActive = timerData['isActive'] ?? false;
        
        print('VLR stream timer update - Countdown: $countdown, IsActive: $isActive');
                        ...
                        Row(
                          children: [
                            ...
                            StreamBuilder<Map<String, dynamic>?>(
                              stream: _queueService.getVLRTimer(widget.roomId),
                              builder: (context, snapshot) {
                                ...
                                return StreamBuilder<Map<String, dynamic>?>(
                                  stream: _queueService.getVLRLiveUser(widget.roomId),
