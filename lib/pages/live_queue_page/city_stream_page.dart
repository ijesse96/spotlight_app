...
class CityStreamPage extends StatefulWidget {
  final String cityId;
  final String cityName;
  final String state;

  const CityStreamPage({
    super.key,
  ...
  @override
  State<CityStreamPage> createState() => _CityStreamPageState();
}

class _CityStreamPageState extends State<CityStreamPage> {
  ...
  void initState() {
    ...
    _initializeCityStream();
  }

  void _initializeCityStream() async {
    // Initialize city-specific stream (only if timer doesn't exist)
    ...
    _listenToCityLiveUser();
    _listenToCityTimer();
  }

  void _listenToCityLiveUser() {
    _liveUserSubscription = _queueService.getCityLiveUser(widget.cityId).listen((liveUser) {
      if (liveUser != null) {
        final userName = liveUser['name'] as String?;
        setState(() {
          _currentLiveUserName = (userName != null && userName.isNotEmpty) ? userName : "Unknown User";
        });
      } else {
        setState(() {
  ...
  void _listenToCityTimer() {
    _timerSubscription = _queueService.getCityTimer(widget.cityId).listen((timerData) {
      if (timerData != null) {
        final countdown = timerData['countdown'] ?? 20;
        final isActive = timerData['isActive'] ?? false;
        
        print('City stream timer update - Countdown: $countdown, IsActive: $isActive');
        ...
        if (isActive) {
          ...
          print('City stream timer is not active, stopping countdown');
          // The persistent timer in QueueService handles this
        }
      }
    });
                        ...
                        Row(
                          children: [
                            ...
                            StreamBuilder<Map<String, dynamic>?>(
                              stream: _queueService.getCityTimer(widget.cityId),
                              builder: (context, snapshot) {
                                ...
                                final int countdown = data['countdown'] ?? 20;
                                final bool isActive = data['isActive'] ?? false;
                                
                                // Check if there's a live user to determine if timer should be active
                                return StreamBuilder<Map<String, dynamic>?>(
                                  stream: _queueService.getCityLiveUser(widget.cityId),
                                  builder: (context, liveUserSnapshot) {
                                    ...
                                    return Text(
                                      '$countdown s',
                                      style: TextStyle(
                                        color: shouldShowActive ? Colors.white : Colors.white54,
                                        fontSize: 12,
                                      ),
                                    );
                                  },
